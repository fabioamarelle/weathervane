...
import com.ra1.aplicaciotemps.utilities.api.Forecast;
import com.ra1.aplicaciotemps.utilities.api.Geocoding;
import com.ra1.aplicaciotemps.utilities.LocationManager;
import com.ra1.aplicaciotemps.R;
import com.ra1.aplicaciotemps.utilities.api.Weather;
...
public class MainActivity extends BaseActivity {
    ...
    private void updateWeatherData() {
        ...
        new Thread(() -> {
            final String weatherResult = Weather.getWeather(userCoordinates, unit, lang);
            final String forecastResult = Forecast.getForecast(userCoordinates, unit, lang);
    ...
    protected void showWeather(String result) {
        ...
        try {
            ...
            long tMin = Math.round(weatherData.getJSONObject("main").getDouble("temp_min"));
            long tMax = Math.round(weatherData.getJSONObject("main").getDouble("temp_max"));
            this.rangTemperatura.setText(getString(R.string.temp_range_format, tMin, tMax));

            long tFeels = Math.round(weatherData.getJSONObject("main").getDouble("feels_like"));
            this.sensacioTermica.setText(getString(R.string.feels_like_format, tFeels));
    ...
    protected void showForecast(String result) {
        if (result.startsWith("Error") || result.equals("-")) return;
        try {
            JSONObject weatherData = new JSONObject(result);
            ...
            TextView[] dates = {previsioData1, previsioData2, previsioData3};
            for (TextView dateView : dates) {
                calendar.add(Calendar.DAY_OF_YEAR, 1);
                dateView.setText(String.format(Locale.getDefault(), "%02d/%02d", calendar.get(Calendar.DAY_OF_MONTH), calendar.get(Calendar.MONTH) + 1));
            }
        } catch (JSONException e) {
    ...
    private void updateBackgroundAndImage(String apiWeather, ImageView imageView) {
        ...
        int bgId = R.drawable.screen_background_cloudy;
        switch (apiWeather) {
            case "Clear": bgId = R.drawable.screen_background_clear; break;
            case "Rain": case "Drizzle": case "Thunderstorm": bgId = R.drawable.screen_background_rain; break;
        }
        this.screenView.setBackground(ContextCompat.getDrawable(getApplicationContext(), bgId));
    }
